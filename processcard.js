...
/*
    It helps in processing monster cards
*/
const processcard = class processcard{
    constructor(jsonDataSet){
        ...
        jsonDataSet.data.forEach(card =>{
            this.mapOfCardInformations.set(card.name, new monstercard(card.name, card.atk, card.def, card.level, card.attribute, card.card_prices, card.card_images));
        })
    }
    ...
    getImageCard(cardName){
        if(this.doesTheCardExistInCache(cardName)){
            return this.mapOfCardInformations.get(cardName).card_images
        }else{
            return null
        }
    }
